                        ...
                        <div class=" overflow-x-auto">
                          <div class="w-full">
                              <div class="relative text-gray-500 focus-within:text-gray-900 mb-4 flex justify-between items-center">
                                  ...
                                  <a href="{{ route('admin.articles.create') }}" class="text-black bg-transparent border border-black hover:bg-black hover:text-white duration-100 font-bold py-2 px-4 rounded-full">
                                      {{ __('Ajouter') }}
                                  </a>
                              </div>
                                        ...
                                        @foreach ($articles as $article)
                                            <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <img src="{{ asset('assets/images/articles/'.$article->image) }}" alt="{{ $article->titre }}" class="w-10 h-10">
                                                </td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 ">{{ Str::limit($article->titre, 30) }}</td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ \Carbon\Carbon::parse($article->created_at)->format('d-m-y') }}</td>
                                                <td class="p-5">
                                                    <div class="flex items-center gap-1">
                                                        <a href="{{ route('admin.articles.edit', $article->id) }}" class="p-2 rounded-full group transition-all duration-500 flex items-center">
                                                        ...
                                                        <button
                                                            class="p-2 rounded-full group transition-all duration-500 flex items-center text-red-600 hover:bg-red-100 delete-button"
                                                            data-id="{{ $article->id }}">
                                                        ...
                                                        <form id="delete-form-article-{{ $article->id }}" action="{{ route('admin.articles.destroy', $article->id) }}" method="POST" style="display: none;">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                                                    </div>
    ...
    <script>
        document.querySelectorAll('.delete-button').forEach(button => {
            button.addEventListener('click', function (event) {
                ...
                if (confirm('{{ __('Voulez-vous vraiment supprimer cet article ?') }}')) {
                    document.getElementById(`delete-form-article-${articleId}`).submit();
                }
            });
        });
    </script>
