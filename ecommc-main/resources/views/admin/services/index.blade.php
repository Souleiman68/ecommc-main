                ...
                <div class="p-6 text-gray-900">
                    <div class="flex flex-col">
                        <div class="overflow-x-auto">
                            <div class="w-full">
                                <div class="relative text-gray-500 focus-within:text-gray-900 mb-4 flex justify-between items-center">
                                    <div class="flex-grow">
                                        ...
                                        <input type="text" id="default-search" class="block w-80 h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none" placeholder="{{ __('Rechercher...') }}">
                                    </div>
                                    <a href="{{ route('admin.services.create') }}" class="text-black bg-transparent border border-black hover:bg-black hover:text-white duration-100 font-bold py-2 px-4 rounded-full">
                                        {{ __('Ajouter') }}
                                    </a>
                                </div>
                                    ...
                                    <table class="min-w-full rounded-xl">
                                        <thead>
                                            <tr class="bg-gray-50">
                                                ...
                                                <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize">Titre</th>
                                                <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize">Prestataire</th>
                                                <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize">Localisation</th>
                                                <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize">Date publication</th>
                                                ...
                                                @foreach ($services as $service)
                                                    <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                                                        <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                            <img src="{{ asset('assets/images/services/'.$service->image) }}" alt="{{ $service->titre }}" class="w-10 h-10">
                                                        </td>
                                                        <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ Str::limit($service->titre, 30) }}</td>
                                                        <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                            {{ $service->provider->nom_complet }} <!-- Afficher le nom du prestataire -->
                                                        </td>
                                                        <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                            {{ $service->localisation }} <!-- Afficher la localisation -->
                                                        </td>
                                                        <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ \Carbon\Carbon::parse($service->created_at)->format('d-m-y') }}</td>
                                                        <td class="p-5">
                                                            <div class="flex items-center gap-1">
                                                                <a href="{{ route('admin.services.edit', $service->id) }}" class="p-2 rounded-full group transition-all duration-500 flex items-center">
                                                                ...
                                                                <button class="p-2 rounded-full group transition-all duration-500 flex items-center text-red-600 hover:bg-red-100 delete-button" data-id="{{ $service->id }}">
                                                                ...
                                                                <form id="delete-form-service-{{ $service->id }}" action="{{ route('admin.services.destroy', $service->id) }}" method="POST" style="display: none;">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                </form>
                                                            </div>
        ...
        document.querySelectorAll('.delete-button').forEach(button => {
            button.addEventListener('click', function (event) {
                ...
                if (confirm('{{ __('Voulez-vous vraiment supprimer ce service ?') }}')) {
                    document.getElementById(`delete-form-service-${serviceId}`).submit();
                }
            });
        });
    </script>
