<x-principale-layout>
    <x-slot:title>
        {{ __('Ebnete Cosmetique - Tous nos services') }}
    </x-slot:title>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 my-8">
            @foreach ($services as $service)
                <div class="bg-white rounded-xl shadow-lg hover:shadow-2xl transition duration-300 overflow-hidden border border-gray-200">
                    ...
                    <!-- Contenu du service -->
                    <div class="p-6">
                        <a href="{{ route('show.service', $service) }}" class="text-xl font-bold text-gray-900 hover:text-indigo-800 transition duration-300">
                            {{ $service->titre }}
                        </a>
